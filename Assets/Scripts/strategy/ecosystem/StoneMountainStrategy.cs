...
public class StoneMountainStrategy :EcosystemStrategy//草原构造
{
    ...
    public override IEnumerator ConstructEcosystem(GameObject parent, Vector2 ChunkPosition, ECube[,,] ECubeList)
    {
        ...
        while (position.Count != 0)
        {
            for (int i = 0; i < position.Count; i++)
            {
                ...
                ECubeList[(int)position[i].x, (int)position[i].y, (int)heights[(int)position[i].x, (int)position[i].y]] = ECube.STONE;
                GameObject temp;
