...
public class GrasslandStrategy : EcosystemStrategy//草原构造
{
    ...
    const int GrassminHeight = 12;//草地的最小高度
    ...
    public override IEnumerator ConstructEcosystem(GameObject parent, Vector2 ChunkPosition, ECube[,,] ECubeList)
    {
        ...
        while (position.Count != 0)
        {
            for (int i = 0; i < position.Count; i++)
            {
                ...
                GameObject temp;
                if (heights[x, y] >= GrassminHeight)//草地
                {
                    ECubeList[(int)position[i].x, (int)position[i].y, (int)heights[(int)position[i].x, (int)position[i].y]] = ECube.GREEN_GRASS;
                    temp = PrefabsManagerSystem.GetObjectFromID(ECube.GREEN_GRASS);
                }
                else
                {
                    ECubeList[(int)position[i].x, (int)position[i].y, (int)heights[(int)position[i].x, (int)position[i].y]] = ECube.DIRCUBE;
                    temp = PrefabsManagerSystem.GetObjectFromID(ECube.DIRCUBE);
                }
