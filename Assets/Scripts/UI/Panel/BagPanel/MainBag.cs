...
{
    [SerializeField] FastBag fastBag;
    ...
    void ButtonOnDoubleLeftClicked(int id)
    {
        ...
        if (eCube != ECube.NORMAL)
        {
            int maxConbine = CubeAttrManager.GetCubeAttr(eCube).maxCombine;
            if(objnum==maxConbine)
            {
                return;
            }
            int remindnum = maxConbine - objnum;
            for(int i=0;i<bagCells.Length;i++)
            {
                if(i!=id)
                {
                    if(bagCells[i].GetCurrECube()==eCube)
                    {
                        int tempnum = bagCells[i].GetObjNum();
                        if(tempnum>=remindnum)
                        {
                            bagCells[i].SetObj(eCube, tempnum - remindnum);
                            bagCells[id].AddNum(remindnum);
                            return;
                        }
                        else
                        {
                            bagCells[i].SetObj(eCube, 0);
                            bagCells[id].AddNum(tempnum);
                            remindnum -= tempnum;
                        }
                    }
                }
    ...
    void ShiftLeftClick(int id)//左键+Shift单击
    {
        ...
        if (eCube != ECube.NORMAL)
        {
            int remindnum = fastBag.FastAddObj(eCube, objnum);
            Debug.Log(remindnum);
            bagCells[id].SetObj(eCube, remindnum);
        }
    }
    ...
    void DoubleLeftClick(int id)//左键快速双击
    {
